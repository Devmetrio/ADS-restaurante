...
include_once($_SERVER['DOCUMENT_ROOT'] . '/src/modelo/Boleta.php');
...
class controlComprobante
{
    ...
    public function insertarDatosEnBoleta($dni, $idControlOrden)
    {
        echo "ejecutando insert a boleta<br>";

        // Convertir a enteros
        $dni = (int)$dni;
        $idControlOrden = (int)$idControlOrden;

        $boletaObject = new Boleta();
        $boletaObject->insertarBoletaDatos($dni, $idControlOrden);
        ...
        if ($boletaObject) {
            ...
            echo "Error al insertar boleta<br>";
        }
    }
}
